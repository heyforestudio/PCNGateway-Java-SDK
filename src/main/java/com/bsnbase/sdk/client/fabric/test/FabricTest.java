...
public class FabricTest {
    ...
    public void initConfig() throws IOException {
        ...
        config.setAppCode("app0003202006221525178604850");
        config.setUserCode("USER0003202006220335261962610");
        config.setApi("https://ningboglobalnode.bsngate.com:17602");

        ClassLoader classLoader = getClass().getClassLoader();
        config.setCert(Common.readLocalFile(classLoader.getResource("cert/bsn_gateway_https.crt").getPath()));
        config.setPrk(Common.readLocalFile(classLoader.getResource("cert/private_key.pem").getPath()));
        config.setPuk(Common.readLocalFile(classLoader.getResource("cert/gateway_public_cert_secp256r1.pem").getPath()));

        // config.setTestServerIdn(false);
        config.setMspDir("test");
